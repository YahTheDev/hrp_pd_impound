$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://HRP:ESX/escape', JSON.stringify({}));
		}
	};
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://HRP:ESX/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://HRP:ESX/unimpound', JSON.stringify(plate));
	});

	$('table').on('click', '.unlock', function () {
		var plate = $(this).parent().parent().find('#plate').text();
		$.post('http://HRP:ESX/unlock', JSON.stringify(plate));
	});
